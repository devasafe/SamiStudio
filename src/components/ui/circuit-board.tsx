...
function CircuitBoard({
  ...
  // Theme-aware color defaults. Com `variant` explícito o tema é derivado
  // direto; só o modo "auto" precisa observar o documento.
  const [systemDark, setSystemDark] = React.useState(true);
  const isDark = variant === "auto" ? systemDark : variant === "dark";
  ...
  React.useEffect(() => {
    ...
    const checkTheme = () => {
      ...
      setSystemDark(isDarkMode);
    };

    // Estado inicial fora do tick síncrono (react-hooks/set-state-in-effect).
    const frame = requestAnimationFrame(checkTheme);
